...
const createPost = async (req, res) => {
    try {
        ...
        const {subject, grade, timeSlot, day, startDate, endDate, requirements, expectedPrice} = req.body;
        ...
        const eligibleTutors = await findEligibleTutors(tutors, studentId, timeSlot, day, startDate, endDate, expectedPrice);

        if (eligibleTutors.length === 0) {
            // Không tìm thấy giảng viên phù hợp, tìm thời gian thay thế
            const alternativeTimes = await findAlternativeTimes(subject, grade, day, expectedPrice, studentId, timeSlot);
...
const findEligibleTutors = async (tutors, studentId, timeSlot, day, startDate, endDate, expectedPrice) => {
    try {
        // Filter tutors based on price first if expectedPrice is set
        let eligibleTutors = tutors;
        if (expectedPrice > 0) {
            eligibleTutors = tutors.filter(tutor => tutor.classPrice <= expectedPrice);
        }
        ...
        for (const tutor of eligibleTutors) {
            ...
            const hasAvailableSchedule = daySchedule && daySchedule.timeSlots.includes(timeSlot);
            ...
            if (potentialConflictClasses.length === 0 && studentConflictClasses.length === 0) {
...
const calculateTutorScore = (tutor, expectedPrice) => {
    ...
    if (expectedPrice > 0) {
        const priceDifference = expectedPrice - tutor.sessionPrice;
...
const findAlternativeTimes = async (subject, grade, preferredDay, expectedPrice, studentId, preferredTimeSlot) => {
    try {
        const availableSlots = [
            "7:00-9:00",
            "9:30-11:30",
        ...
        for (const tutor of eligibleTutors) {
            ...
            for (const slot of tutorAvailableSlots) {
                ...
                const isPriceWithinBudget = expectedPrice === 0 || tutor.sessionPrice <= expectedPrice;
                ...
                for (const classItem of tutorClasses) {
                    if (classItem.timeSlot === slot.timeSlot && classItem.day === slot.day) {
        ...
        alternatives.sameDayDifferentTime.sort((a, b) => a.sessionPrice - b.sessionPrice);
        combinedAlternatives = [...combinedAlternatives, ...alternatives.sameDayDifferentTime];

        // 2. Cùng ca, khác ngày
        alternatives.sameTimeDifferentDay.sort((a, b) => a.sessionPrice - b.sessionPrice);
        combinedAlternatives = [...combinedAlternatives, ...alternatives.sameTimeDifferentDay];

        // 3. Cùng ngày và ca nhưng giá cao hơn
        alternatives.sameDayTimeHigherPrice.sort((a, b) => a.sessionPrice - b.sessionPrice);
        combinedAlternatives = [...combinedAlternatives, ...alternatives.sameDayTimeHigherPrice];
        ...
        combinedAlternatives = [...combinedAlternatives, ...alternatives.otherOptions];
...
const acceptPost = async (req, res) => {
    try {
        ...
        const tutor = await Tutor.findById({userId}).populate("userId");
        ...
        if (!post.currentAssignedTutor || post.currentAssignedTutor.toString() !== tutor._id.toString()) {
        ...
        const totalClasses = getNumberOfClassesInDateRange(post.startDate, post.endDate, post.day);

        // Trừ tiền từ tài khoản học viên
        if (student.balance < tutor.sessionPrice * totalClasses) {
            return res.status(400).json({
                status: "fail",
        ...
        tutor.responseRate = calculateNewResponseRate(tutor.responseRate || 0, true);
        await tutor.save();
...
const rejectPost = async (req, res) => {
    try {
        ...
        // Check if this tutor is currently assigned to this post
        if (!post.currentAssignedTutor || post.currentAssignedTutor.toString() !== tutorId.toString()) {
            return res.status(403).json({
                status: "fail",
                message: "Bạn không được chỉ định cho bài đăng này",
            });
        ...
        const nextTutor = post.eligibleTutors.find(eligible =>
            eligible.tutorId.toString() !== tutorId.toString() &&
            !post.rejectedTutors.some(rejected => rejected.tutorId.toString() === eligible.tutorId.toString())
        );
        ...
        if (nextTutor) {
            ...
            if (tutor) {
                ...
                tutor.responseRate = calculateNewResponseRate(tutor.responseRate || 0, false);
                await tutor.save();
            }
...
const checkExpiredRequests = async () => {
    try {
        ...
        for (const post of expiredPosts) {
            ...
            const nextTutor = post.eligibleTutors.find(eligible =>
                eligible.tutorId.toString() !== post.currentAssignedTutor.toString() &&
                !post.rejectedTutors.some(rejected => rejected.tutorId.toString() === eligible.tutorId.toString())
            );
...
const selectAlternativeTime = async (req, res) => {
    try {
        ...
        if (!post.alternativeTimes || alternativeIndex >= post.alternativeTimes.length) {
            return res.status(400).json({
                status: "fail",
        ...
        const newClass = new Class({
            ...
            grade: post.grade,
            requirements: post.requirements,
            classPrice: selectedAlternative.sessionPrice,
            status: "active"
        });
...
const cancelPost = async (req, res) => {
    try {
        ...
        // Kiểm tra quyền truy cập
        if (post.studentId.toString() !== req.user._id.toString() && req.user.role !== "admin") {
            return res.status(403).json({
                status: "fail",
                message: "Bạn không có quyền thực hiện hành động này",
            });
...
const getAssignedPosts = async (req, res) => {
    try {
        ...
        const totalAssignedCount = await Post.countDocuments({
            $or: [
                {currentAssignedTutor: tutor._id, status: "waiting_tutor_confirmation"},
                {
                    status: "matched",
                    "eligibleTutors.tutorId": tutor._id
                }
        ...
        res.json({
            ...
            data: {
                ...
                stats: {
                    ...
                    acceptanceRate: totalAssignedCount > 0 ?
                        Math.round((acceptedCount / (acceptedCount + rejectedCount)) * 100) : 0
                }
            }
        });
    } catch (error) {
