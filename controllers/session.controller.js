...
const completeSession = async (req, res) => {
    try {
        ...
        res.json({
            ...
            data: {
                session,
                trustScoreAdded: rewardPoints + (tutor.consecutiveCompletedSessions % 5 === 0 ? 2 : 0)
            },
        });
    } catch (error) {
        res.status(500).json({
...
const cancelSession = async (req, res) => {
    try {
        ...
        if (userRole === "tutor") {
            ...
            if (session.studentId && session.studentId.toString() !== userId.toString()) {
                return res.status(403).json({
                    status: "fail",
                    message: "Bạn không có quyền hủy buổi học này",
                });
        ...
        if (canceledBy === "tutor") {
            ...
            const now = new Date();
            const hoursRemaining = Math.max(0, (sessionStartDate - now) / (1000 * 60 * 60));
