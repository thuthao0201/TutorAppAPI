...
const getTutors = async (req, res) => {
  try {
    ...
    switch (sortField) {
      case 'rating':
        tutorsWithFollowStatus.sort((a, b) => (a.avgRating - b.avgRating) * sortOrder);
        break;
      case 'price':
        tutorsWithFollowStatus.sort((a, b) => (a.sessionPrice - b.sessionPrice) * sortOrder);
        break;
      case 'newest':
        tutorsWithFollowStatus.sort((a, b) => (new Date(a.createdAt) - new Date(b.createdAt)) * sortOrder);
        break;
      case 'sessions':
      case 'completedClasses':
        tutorsWithFollowStatus.sort((a, b) => (a.completedClasses - b.completedClasses) * sortOrder);
        break;
      case 'consecutive':
      case 'consecutiveCompletedClasses':
        tutorsWithFollowStatus.sort((a, b) => (a.consecutiveCompletedClasses - b.consecutiveCompletedClasses) * sortOrder);
        break;
      default:
    ...
    if (isFeatured === 'true') {
      ...
      tutorsWithFollowStatus = featuredTutors.filter((tutor, index) =>
  ...
  try {
    ...
    if (req.user) {
      ...
      if (req.user.role === 'student') {
        ...
        const existingReview = await mongoose.model('Review').exists({
          tutorId: tutor._id,
          userId: req.user._id
        });
